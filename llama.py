...
# ==================== 配置 ====================
MODEL_PATH = "/root/autodl-tmp/atom_model"   # 请修改为你的模型实际路径
USE_4BIT = False
...
PERSONAL_EXAMPLES = [
    ...
    {"field": "个人银行账号", "category": "客户个人信息"},  # 客户可能使用个人账号
    {"field": "家庭住址", "category": "客户个人信息"},
    {"field": "报账单据", "category": "客户个人信息"},  # 报账单据可能涉及客户信息
...
def classify_field(model, tokenizer, prompt, categories, temperature=0.0):
    ...
    # 可选调试输出（取消注释可查看模型实际输出）
    # print(f"生成文本: {generated_text}")
    for cat in categories:
        if cat in generated_text:
            return cat
    # 如果还是没匹配，尝试在完整输出中查找（最后手段）
...
def batch_process(input_file, output_file):
    ...
    print(f"共读取 {len(fields)} 个字段")
    model, tokenizer = load_model()

    results = []
    for idx, field in enumerate(fields, 1):
        print(f"处理中 ({idx}/{len(fields)}): {field}")
        # 第一层分类
        ...
        if level1 == "企业数据":
            ...
            level2 = "解析失败"
            results.append((field, level1, level2))
            continue
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        for field, l1, l2 in results:
            f.write(f"{field}\t{l1}\t{l2}\n")
...
def main():
    if len(sys.argv) < 2:
        print("用法: python script.py 输入文件名 [输出文件名]")
        print("示例: python script.py fields.txt results.txt")
        sys.exit(1)
    ...
    if len(sys.argv) >= 3:
        ...
        output_file = f"{base}_result.txt"
